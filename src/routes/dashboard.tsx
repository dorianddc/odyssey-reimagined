...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Tableau de bord analytique de la progression des élèves en EPS.",
      },
    ],
  }),
...
function DataDashboard() {
  ...
  return (
    <main className="min-h-screen px-4 md:px-8 py-8 md:py-12">
      ...
      {!mounted ? (
        <div className="max-w-7xl mx-auto py-20 text-center text-ink-soft font-semibold">
          Chargement…
        </div>
      ) : (
        <div className="max-w-7xl mx-auto space-y-8">
          <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KpiCard
              ...
              label="Niveau Moyen"
              value={kpis.avgLevel}
              sub="MOYENNE + ARRONDI(1)"
              tone="info"
            />
            <KpiCard
              icon={<AlertTriangle />}
              label="Élèves en Alerte"
              value={kpis.enAlerte}
              sub="NB.SI(difficultés ≥ 1)"
              tone="hot"
            />
            <KpiCard
              icon={<Crown />}
              label="Taux de Maîtrise"
              value={`${kpis.tauxMaitrise}%`}
              sub="SI(niveau > 12)"
              tone="gold"
            />
          </section>
          ...
          <section className="rounded-[var(--radius)] border-[3px] border-ink bg-surface shadow-pop overflow-hidden">
            <div className="flex flex-wrap items-center gap-3 p-4 border-b-[3px] border-ink bg-surface-2">
              ...
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-ink-soft">
                ...
                <Select
                  value={genderFilter}
                  onValueChange={(v) => setGenderFilter(v as "all" | "F" | "M")}
                >
              ...
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-ink-soft">
                ...
                <Select
                  value={statusFilter}
                  onValueChange={(v) => setStatusFilter(v as "all" | "diff" | "ok")}
                >
              ...
              <table className="w-full text-sm">
                <thead className="bg-surface-2 border-b-[3px] border-ink">
                  <tr className="text-left font-display uppercase tracking-wider text-[11px]">
                    ...
                    <Th onClick={() => toggleSort("gender")}>
                      Genre <SortIcon k="gender" />
                    </Th>
                    <Th onClick={() => toggleSort("level")}>
                      Niveau Global <SortIcon k="level" />
                    </Th>
                    <Th onClick={() => toggleSort("diffs")}>
                      Difficultés <SortIcon k="diffs" />
                    </Th>
                    <Th onClick={() => toggleSort("tier")}>
                      Dernier Palier <SortIcon k="tier" />
                    </Th>
                  </tr>
                </thead>
                <tbody>
                  {tableRows.length === 0 && (
                    <tr>
                      <td colSpan={5} className="p-8 text-center text-ink-soft font-semibold">
                  ...
                  {tableRows.map((r) => (
                    <tr
                      key={r.id}
                      className={cn(
                        "border-b border-ink/10 hover:bg-surface-2/60 transition-colors",
                        rowAlertClass(r.diffs),
                      )}
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={cn(
                            "inline-block px-2.5 py-1 rounded-lg text-xs font-display border-2",
          ...
          <section className="grid grid-cols-1 gap-6">
            <div className="rounded-[var(--radius)] border-[3px] border-ink bg-surface shadow-pop p-5">
              ...
              <p className="text-xs text-ink-soft font-semibold mb-4">
                NB.SI(niveau ∈ [min, max]) pour chaque palier
              </p>
  ...
  const handleImportFile = (file: File) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (data[LS_CLASSES] !== undefined)
          localStorage.setItem(LS_CLASSES, JSON.stringify(data[LS_CLASSES]));
        if (data[LS_STUDENTS] !== undefined)
          localStorage.setItem(LS_STUDENTS, JSON.stringify(data[LS_STUDENTS]));
        if (data[LS_HISTORY] !== undefined)
          localStorage.setItem(LS_HISTORY, JSON.stringify(data[LS_HISTORY]));
    ...
    <section className="rounded-[var(--radius)] border-[3px] border-ink bg-surface shadow-pop p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-display text-lg tracking-wide">
          Gestionnaire de base (Sauvegarde / Restauration)
        </h3>
      </div>
      <p className="text-xs text-ink-soft font-semibold mb-4">
        Exporte ou restaure les 3 clés persistées (classes, élèves, historique des situations) en un
        fichier JSON unique.
      </p>
    ...
    <div className="rounded-[var(--radius)] border-[3px] border-ink bg-surface shadow-pop p-5 relative overflow-hidden">
      ...
      <div className="relative">
        <div className="flex items-center gap-2 mb-2">
          <div
            className={cn(
              "w-9 h-9 grid place-items-center rounded-xl border-2 border-ink bg-surface",
              toneCls[tone].split(" ").pop(),
            )}
          >
            {icon}
          </div>
